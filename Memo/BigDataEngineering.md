...
### 1.2	HDFS在读取文件的时候,如果其中一个块突然损坏了怎么办
...
### 1.3	HDFS在上传文件的时候,如果其中一个DataNode突然挂掉了怎么办
...
### 1.5	Secondary NameNode了解吗，它的工作机制是怎样的
...
### 1.6	Secondary NameNode不能恢复NameNode的全部数据，那如何保证NameNode数据存储安全？
...
### 1.7	在NameNode HA中，会出现脑裂问题吗？怎么解决脑裂
...
4. **Merge阶段**：在 ReduceTask 远程复制数据的同时，会在后台开启两个线程对内存到本地的数据文件进行合并操作
5. **Sort阶段**：在对数据进行合并的同时，会进行排序操作，由于 MapTask 阶段已经对数据进行了局部的排序，ReduceTask 只需保证 Copy 的数据的最终整体有效性即可。

> Shuffle 中的缓冲区大小会影响到 mapreduce 程序的执行效率，原则上说，缓冲区越大，磁盘io的次数越少，执行速度就越快
> 缓冲区的大小可以通过参数调整, 参数：mapreduce.task.io.sort.mb 默认100M

### 1.13	shuffle阶段的数据压缩机制了解吗？
...
- `Hadoop Yarn` 模式：资源管理器是 `Yarn` 集群，支持动态资源管理，还可以调度其他实现了 `Yarn` 调度接口的集群计算，非常适用于多个集群同时部署的场景，是较为流行的一种资源管理系统；
- `Apache Mesos` 模式：是专门用于分布式系统资源管理的开源系统，可以对集群中的资源做弹性管理。
...
DAG调度需要对计算过程划分 `stage`，划分的依据就是 `RDD` 之间的依赖关系，划分的指标是看父`RDD` 的 `Partition` 做多被几个子`RDD` 使用
...
scala程序必须从一个对象的main方法开始，一个scala应用程序只能有一个入口

Scala不用class作为类的关键字，而是使用object，表示静态类
...
- 用户的标签会非常多，而且随着用户画像的深入，会有很多细分领域的标签，这意味着标签的数量会随时增加，而且可能会很频繁；
- 不同的标签计算频率不同，需要按照不同的周期更新；
- 计算完成时间不同，横表存储需要把各个小表的计算结果合并，最终表的完成时间取决于最慢的那一张；
- 大量空缺的标签会导致存储系数。

用竖表可以解决这个问题（如HBase）但会带来查询效率下降等新的问题。
