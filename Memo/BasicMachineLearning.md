...
- boosting**串行**训练基分类器，各分类器之间有依赖，基本思路是将基分类器层层叠加，每次迭代对之前分错的样本给与更高权重，测试时根据各层分类器的加权获得最终结果，迭代式学习；

- bagging**并行**训练基分类器，各基分类器之间无强依赖，每个单独学习，最后再投票决策，分而治之学习；
...
- 两者提高基分类器性能的本质（故基分类器最好是本身对样本分布较为敏感的不稳定的）：
  ...
  - Bagging——有效降低方差
    集成投票使得基分类器可能出现的过拟合现象得到了平缓，所以最终模型笔各个独立的模型更**平滑**
...
- 决策树可以较方便地将样本的权重整合到训练过程中，而不需要使用过采样的方法来调整样本权重；
- 可以通过调节树的**层数**来折中平衡决策树的表达能力和泛化能力；
- 数据样本的扰动对决策树影响较大，故不同子样本集合生成的决策树基分类器随机性较大。
- 神经网络也可做基分类器，同样不稳定，且方便调节层数、权值等引入随机性。

基分类器是弱分类器，其错误是偏差和方差两种错误之和。

如果使用线性分类器的较为稳定的基分类器，不同基分类器间方差差异不大，使用bagging集成没啥用。
...
Gradient Boosting的基本思想就是**根据当前模型损失函数的负梯度信息来训练新加入的弱分类器**，然后将训练好的弱分类器以累加的形式结合到现有模型中。

![image-20210823161955298](http://wy-typora-img.oss-cn-chengdu.aliyuncs.com/img/image-20210823161955298.png)
...
监督学习中，模型的泛化误差主要来源于两个方面：偏差与方差。随着模型复杂度的增加，偏差逐步降低，方差逐步升高，总的泛化误差先降再升。
...
最早是信息论领域的香农明确给出了熵的定义：无损编码事件信息的最小平均编码长度。
...
### 详细说一下应对不平衡数据的欠采样和过采样方法？

- 欠采样即去除一部分不平衡样本中较多的实例，代表性算法有 `EasyEnsemble` 算法，利用集成学习机制，将超平衡的类别数据划分成若干个集合供不同学习器使用，这样对每个学习器来说都是欠采样后的平衡样本，但在全局看来未丢失重要信息；
- 过采样即增加一些欠平衡样本，但不能简单的做重复采样，容易产生严重的过拟合，代表性算法有`SMOTE`，通过对欠平衡样本进行插值来产生额外的数据。
...
机器学习并非数据科学的全部，其一个公平的定义是利用数据创建某种模型的算法，并从中可以得到问题的答案。然后并非所有用用的解决方案都可以用模型来表示，如==局部敏感哈希数据库(LSH)==是一种非常强大的处理数据的技术，但其不属于机器学习方法。

https://mp.weixin.qq.com/s/Lfw_vius6cZ6DVuXsXOLEw