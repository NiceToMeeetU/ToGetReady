...
- 一是梳理业务管理流程框架；
  ...
  我们首先需要梳理业务流程、管理流程、数据流程的具体内容，了解不同环节、不同层次的数据如何产生，又能如何作用于业务决策，先有个宏观认识；
...
- 三是对指标进行评估取舍及归类。
  ...
  需要业务、数据、开发等部门会商，根据紧迫性、开发难度、重要性等

  选择一些成熟的框架进行构建，如AARRR

指标体系的作用：

- 监控业务情况；
- 通过拆解指标寻找当前业务问题；
- 评估业务可改进的地方，找到下一步工作方向。
...
用于比较两个独立样本的差异，若两个样本来自两个独立的但非正态或者形态不清的两总体，则不应该用t检验去判断其样本间的差异是否显著，而需采用秩和检验。

秩检验也称等级检验，是一类非参数检验方法，基于样本观测值的秩的统计检验，其使用的统计量，不是直接由样本观测值计算的，二是由样本观测值的秩计算

具体方法：若样本量都小于10，直接求两个的秩和，查表判断是否拒绝原假设；若样本量都大于10，秩和T的分布接近正态，可以使用Z检验。
